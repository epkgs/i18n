...
var _ HTTPError = (*errWithStatus)(nil)
...
func (d *errorDefinition) WithStatus(code, httpStatus int) *errorDefinition {
	...
	return d.With(ErrWithStatusWrapper(code, httpStatus))
}

// ErrWithStatusWrapper 是一个高阶函数，用于创建一个错误包装器，该包装器会将错误与指定的状态码关联。
...
// 返回值:
//
//	ErrorWrapFunc - 一个函数类型，接受一个错误作为输入，返回一个经过包装后的错误，该错误包含了原始错误、自定义错误码和HTTP状态码。
func ErrWithStatusWrapper(code, httpStatus int) ErrorWrapFunc {
	return func(err error) error {
		return ErrWithStatus(err, code, httpStatus)
	}
}

// ErrWithStatus 是一个用于包装错误信息的函数，它允许错误信息携带额外的状态码和HTTP状态码。
...
func ErrWithStatus(err error, code int, httpStatus int) error {
...
// HttpStatus 返回错误状态码
//
// 该方法用于获取 errWithStatus 类型对象的 HTTP 状态码。
// 它返回一个整数类型的 HTTP 状态码，表示与错误相关的 HTTP 状态。
func (e *errWithStatus) HttpStatus() int {
	return e.httpStatus
}
...
var _ TracedError = (*errWithTraceID)(nil)

type errWithTraceID struct {
	error
...
func (d *errorDefinition) WithTraceID(traceID string) *errorDefinition {
	return d.With(ErrWithTraceIDWrapper(traceID))
}

// ErrWithTraceIDWrapper 是一个返回 ErrorWrapFunc 类型的函数。
// 它的作用是创建并返回一个闭包，该闭包用于将特定的 traceID 与错误信息关联。
// 参数 traceID 是一个字符串，代表了需要与错误信息关联的追踪ID。
func ErrWithTraceIDWrapper(traceID string) ErrorWrapFunc {
	// 返回一个闭包，该闭包接收一个错误 err，并使用 ErrWithTraceID 函数将 traceID 与 err 关联起来。
...
// ErrWithTraceID is a function that wraps an error with a trace ID.
...
func (e *errWithTraceID) TraceID() string {
	return e.traceID
}
